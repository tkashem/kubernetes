...
func TestUnsafeDeletePrecondition(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			opts:          option(true),
			shouldContain: unsafeDeleteNotAllowed,
		},
		{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			_, _, err = deleter.Delete(ctx, "foo", rest.ValidateAllObjectFunc, test.opts)

			switch {
			case len(test.shouldContain) > 0:
				if err == nil || !strings.Contains(err.Error(), test.shouldContain) {
					t.Errorf("Expected error to contain: %q, but got: %v", test.shouldContain, err)
				}
			default:
				if err != nil {
...
func (w deleteWant) verify(t *testing.T, got result) {
	...
	const key = "apiserver.k8s.io/unsafe-delete-ignore-read-error"
	switch w.annotation {
	case true:
		if value, ok := got.event.Annotations[key]; !ok || len(value) > 0 {
			t.Errorf("Expected annotation %q to exist with an empty value, but got exists: %t, value: %q", key, ok, value)
		}
	default:
		if got.event != nil {
			if value, ok := got.event.Annotations[key]; ok || len(value) > 0 {
				t.Errorf("Did not expect annotation %q to exist, but got exists: %t, value: %q", key, ok, value)
			}
		}
	}
}

func enableAudit(ctx context.Context) context.Context {
...
func TestUnsafeDeleteWithCorruptObject(t *testing.T) {
	...
	got := result{event: audit.AuditEventFrom(ctx)}
...
func TestUnsafeDeleteWithUnexpectedError(t *testing.T) {
	...
	want = deleteWant{checkErr: wantErrContains("is exclusively used to delete corrupt object(s), try again by removing this option")}
	want.verify(t, got)
...
func TestUnsafeDeleteWithLatestRevision(t *testing.T) {
	ctx := genericapirequest.WithNamespace(genericapirequest.NewContext(), "test")
	destroyFunc, registry := NewTestGenericStoreRegistry(t)
	defer destroyFunc()
	...
	obj, err := registry.Get(ctx, object.Name, &metav1.GetOptions{})
	if err != nil {
		t.Fatalf("Unexpected error: %v", err)
	}
	rv, err := strconv.Atoi(obj.(*example.Pod).ResourceVersion)
	if err != nil {
		t.Fatalf("Unexpected error getting revision of the object: %v", err)
	}
...
func TestUnsafeDeleteWithOlderRevision(t *testing.T) {
	ctx := genericapirequest.WithNamespace(genericapirequest.NewContext(), "test")
	destroyFunc, registry := NewTestGenericStoreRegistry(t)
	defer destroyFunc()
	...
	// b) get the revision of the target object from the storage
	obj, err := registry.Get(ctx, "foo", &metav1.GetOptions{})
	if err != nil {
		t.Fatalf("Unexpected error from Get: %v", err)
	}
	rv, err := strconv.Atoi(obj.(*example.Pod).ResourceVersion)
	if err != nil {
		t.Fatalf("Unexpected error geeting revision of the obeject: %v", err)
	}

	// c) update the object so it gets a new revision
	object.Spec.NodeName = "another-machine"
	if _, _, err := registry.Update(ctx, "foo", rest.DefaultUpdatedObjectInfo(object), rest.ValidateAllObjectFunc, rest.ValidateAllObjectUpdateFunc, false, &metav1.UpdateOptions{}); err != nil {
	...
	cs := &corruptStorage{
		Interface: registry.Storage.Storage,
		// we trick the Get operation to return an older revision
		err: storage.NewCorruptObjError("key", storage.NewInternalErrorWithRevision(int64(rv), fmt.Errorf("untransformable"))),
	}
	registry.Storage.Storage = cs
	ctx = enableAudit(ctx)
	...
	want := deleteWant{
		deleted:    false,
		annotation: true,
		checkErr:   wantErrContains(fmt.Sprintf("Precondition failed: ResourceVersion in precondition: %d, ResourceVersion in object meta: %d", rv, rv+1)),
	}
	want.verify(t, got)
}
